 ...
 */
public class ActivityMySendRequest extends AppCompatActivity {
    ...
    public void refresh()
    {
        Log.d("NET", "ActivityMySendRequest" + "refresh");
        MessageList.clear();
    ...
    {
        client.get("http://gofish.hackpku.com:8003/api/users/"+USR.usr_id+"/trade_requests/sent", null, new JsonHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, JSONArray response) {
                try {
                    JSONArray list = response;
                    idList.clear();
                    Log.d("NET", "ActivityMySendRequest get success");
                    for (int i = 0; i < list.length(); i++) {
                        idList.add(list.getInt(i));
                        Log.d("NET", "ActivityMySendRequest get " + list.getInt(i));
                    }
                    refresh();
                    mSwipeLayout.setRefreshing(false);
